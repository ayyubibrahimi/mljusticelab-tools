...
from llm import gpt_4o, claude_3_5_sonnet

from tavily_helper import tavily_client, tavily_async_client

# first: scour the web for current events and determines what events of the day warrant further research 
# this program requires sources, such as ddos, wikieaks and yahoo finance.

# second: based on news of the day write a research report

# third: we need a suubequent program that decides to create a bounty given the research report
...
class ReportState(TypedDict):
    topic: str # Report topic    
    sections: list[Section] # List of report sections  f
...
async def main():
    ...
    # Convert configuration to dictionary for LangGraph
    config_dict = {
        "configurable": {
            "report_structure": config.report_structure,
            "number_of_queries": config.number_of_queries,
            "tavily_topic": config.tavily_topic,
            "tavily_days": config.tavily_days
        }
    }
    
    # Execute the graph
    result = await graph.ainvoke(
        {"topic": "Predictions for whether or not Doge coin will gain or lose value over the next 30 days."},
        config=config_dict
    )
