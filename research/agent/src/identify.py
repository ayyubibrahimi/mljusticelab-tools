...
async def fetch_yahoo_finance():
    """Fetch Yahoo Finance navigation content through markdown conversion service"""
    headers = {
    ...
    async with RetryClient(retry_options=retry_options, timeout=timeout) as session:
        async with session.get('https://r.jina.ai/https://finance.yahoo.com/', headers=headers) as response:
            logger.info(f"Yahoo Finance fetch status: {response.status}")
...
async def save_explored_content(explored_content):
        """Save the explored content to a text file"""
        logger.info("Saving explored content to file")
        ...
        try:
            ...
            logger.info("Successfully saved content to explored_content.txt")
...
async def main():
    try:
        # 1. Fetch Yahoo Finance navigation
        logger.info("Starting Yahoo Finance exploration")
        markdown_content = await fetch_yahoo_finance()
