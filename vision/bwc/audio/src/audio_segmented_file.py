...
def process_file(input_dir, output_dir, filename, pipe, segment_duration):
    ...
    try:
        ...
        segments, sr = split_audio_into_segments(preprocessed_audio_path, segment_duration)
        
        all_results = []
        for i, (segment, start_time) in enumerate(segments):
            # Save segment to a temporary file
            temp_segment_file = tempfile.NamedTemporaryFile(suffix=".wav", delete=False)
            sf.write(temp_segment_file.name, segment, sr)
            
            # Process segment
            result = pipe(temp_segment_file.name)
            
            # Adjust timestamps and handle potential None values
            for chunk in result["chunks"]:
                if chunk["timestamp"][0] is not None and chunk["timestamp"][1] is not None:
                    chunk["timestamp"] = (chunk["timestamp"][0] + start_time, chunk["timestamp"][1] + start_time)
                elif chunk["timestamp"][0] is not None:
                    # If end timestamp is None, use the start of the next segment or the end of the audio
                    if i < len(segments) - 1:
                        next_start = segments[i+1][1]
                    else:
                        next_start = len(segment) / sr + start_time
                    chunk["timestamp"] = (chunk["timestamp"][0] + start_time, next_start)
                else:
                    # If both timestamps are None, skip this chunk
                    logging.warning(f"Skipping chunk with no timestamps: {chunk['text']}")
                    continue
            
            all_results.extend(result["chunks"])
            
            # Clean up temporary file
            os.unlink(temp_segment_file.name)
        
        write_transcript_to_file(all_results, output_path)
...
def main():
    ...
    segment_duration = config.get("segment_duration", 60)  # Default to 60 seconds
    ...
    for filename in mp4_files:
        process_file(input_dir, output_dir, filename, pipe, segment_duration)
