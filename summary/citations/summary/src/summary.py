...
def save_summary_to_files(summary_data, output_dir, batch_number):
    """
    Save individual page summary and input text to both JSON and text files.
    
    Args:
        summary_data (dict): Dictionary containing single page data with keys:
            - page_content (str): The summary text
            - page_numbers (int): The page number
            - original_input (str): Original text of the page
        output_dir (str): Directory to save the files
        batch_number (str): Identifier for the batch/page
    """
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_filename = f"page_{summary_data['page_numbers']}_{batch_number}_{timestamp}"
    
    # Save as JSON
    json_path = os.path.join(output_dir, f"{base_filename}.json")
    json_data = {
        "summary": summary_data["page_content"],
        "page_number": summary_data["page_numbers"],
        "original_input": summary_data["original_input"]
    }
    ...
    with open(text_path, 'w', encoding='utf-8') as f:
        # Write summary
        f.write(f"Summary for page {summary_data['page_numbers']}\n")
        f.write("=" * 80 + "\n\n")
        f.write(summary_data["page_content"])
        f.write("\n\n" + "=" * 80 + "\n\n")
        
        # Write original input
        f.write("ORIGINAL INPUT TEXT\n")
        f.write("=" * 80 + "\n\n")
        f.write(f"PAGE {summary_data['page_numbers']}\n")
        f.write("-" * 80 + "\n")
        f.write(summary_data["original_input"])
        f.write("\n" + "-" * 80 + "\n\n")

def save_master_files(all_summaries, output_dir):
    """
    Save all page summaries to master files, sorted by page numbers
    
    Args:
        all_summaries (list): List of dictionaries, each containing single page data
        output_dir (str): Directory to save the files
    """
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    # Sort summaries by page number
    sorted_summaries = sorted(all_summaries, key=lambda x: x["page_numbers"])
    
    # Save master JSON
    master_json_path = os.path.join(output_dir, f"master_summary_{timestamp}.json")
    master_json = {
        "summaries": [
            {
                "summary": summary["page_content"],
                "page_number": summary["page_numbers"],
                "original_input": summary["original_input"]
            }
    ...
    # Save master text file
    master_text_path = os.path.join(output_dir, f"master_summary_{timestamp}.txt")
    with open(master_text_path, 'w', encoding='utf-8') as f:
        f.write("COMPLETE DOCUMENT SUMMARY\n")
        f.write("=" * 80 + "\n\n")
        
        for summary in sorted_summaries:
            f.write(f"Summary for page {summary['page_numbers']}\n")
            f.write("-" * 80 + "\n\n")
            ...
            f.write(f"PAGE {summary['page_numbers']}\n")
            f.write("-" * 80 + "\n")
            f.write(summary["original_input"])
            f.write("\n" + "-" * 80 + "\n\n")
...
def generate_summaries(docs, memory_log, batch_size=20, num_pages_to_concat=10):
    ...
    all_complete_summaries = []  # List to collect full summaries with original input
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            batch_results, complete_results = future.result()
            results.extend(batch_results)
            all_complete_summaries.extend(complete_results)
    
    return results, all_complete_summaries
...
def process_interval(interval_index, start_index, interval_size, docs, memory_log):
    ...
    page_summaries, complete_summaries = generate_summaries(interval_docs, memory_log)
    combined_summary, _ = process_summaries(page_summaries, memory_log)

    return combined_summary, complete_summaries
...
def process_file(filename, input_directory, output_directory, memory_log):
    ...
    num_intervals = 4
    all_complete_summaries = []  # Collect all summaries across intervals
    ...
    # Extract summaries and complete results
    interval_summaries = []
    for combined_summary, complete_summaries in interval_results:
        interval_summaries.append(combined_summary)
        all_complete_summaries.extend(complete_summaries)

    # Save master files after all intervals are processed
    output_dir = os.path.join(output_directory, "complete")
    os.makedirs(output_dir, exist_ok=True)
    save_master_files(all_complete_summaries, output_dir)
    ...
    while i < len(batch):
        ...
        while len(concat_pages) < num_pages_to_concat and i < len(batch):
            ...
            if "No data to be processed on this page" not in current_page:
                ...
                original_pages.append(batch[i].page_content)
            i += 1
        ...
        if concat_pages:
            ...
            # Create individual entries for each page
            for idx, (page_number, original_page) in enumerate(zip(page_numbers, original_pages)):
                page_data = {
                    "page_content": verified_summary,
                    "page_numbers": page_number,
                    "original_input": original_page
                }
                complete_results.append(page_data)
                
                # Also save individual files
                save_summary_to_files(page_data, output_dir, f"{batch_number}_{idx + 1}")
            
            # For backward compatibility, keep the original format as well
            results.append({
                "page_content": verified_summary,
...
def save_summary_to_text(summary, output_path, filename, timestamp):
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Create a specific output file with timestamp
        output_file_path = os.path.join(output_path, f"summary_{timestamp}.json")
        ...
        for filename in os.listdir(input_directory):
            if filename.endswith(".json"):
                ...
                # Save text file
                save_summary_to_text(condensed_summary, output_path, filename, timestamp)
                
                output_data.append(save_summaries_to_json(condensed_summary, filename, start_page, end_page))
        
        # Write to the specific output file
