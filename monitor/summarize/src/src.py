...
def main():
    ...
    # Read the input CSV
    input_csv_path = '../../download/data/output/processed_index.csv'
    
    # Create new output path with timestamp
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_csv_path = f'../data/output/processed_index_with_summaries_{timestamp}.csv'
    
    try:
        # Read CSV with string dtypes for text columns
    ...
    output_directory = "../data/output/summaries"
    os.makedirs(output_directory, exist_ok=True)
    
    # Initialize new columns with explicit string/object type
    text_columns = ['final_summary', 'condensed_final_summary']
    for col in text_columns:
        if col not in df.columns:
            df[col] = pd.Series(dtype='object')
        else:
            # Convert existing columns to object type if they exist
            df[col] = df[col].astype('object')
    
    total_files = len(df)
    processed_files = 0

    # Update file paths to match directory structure
    df['local_pdf_path'] = df['local_pdf_path'].apply(
        lambda x: x.replace("../data", "../../download/data") if pd.notna(x) else x
    )
    
    # Create a copy of the dataframe for processing
    df_processed = df.copy()
    
    # Ensure all text columns are of type object in the processed DataFrame
    for col in text_columns:
    ...
    for idx, row in df.iterrows():
        ...
        if pd.isna(pdf_path):
            print(f"Skipping row {idx}: No PDF path")
            continue
        ...
        try:
            ...
            if pd.notna(row.get('final_summary')) and pd.notna(row.get('condensed_final_summary')):
                print(f"Skipping {filename} as it's already been processed.")
                continue
            ...
            # Explicitly convert memory_log to string if it isn't already
            memory_log = str(memory_log) if memory_log is not None else None
            ...
            final_summaries = final_summarization(file_summaries, memory_log)
            
            # Get concatenated content from final summaries
            final_summary_content = "\n\n".join([summary for summary in final_summaries])
            
            # Generate condensed summary
            condensed_summary = organize_final_summary(final_summaries)
            
            # Clean up summaries before storing
            final_summary_cleaned = clean_summary(final_summary_content)
            condensed_summary_cleaned = clean_summary(condensed_summary)
            
            # Update DataFrame with cleaned summaries, ensuring string type
            df_processed.at[idx, 'final_summary'] = str(final_summary_cleaned) if final_summary_cleaned is not None else None
            df_processed.at[idx, 'condensed_final_summary'] = str(condensed_summary_cleaned) if condensed_summary_cleaned is not None else None
            
            processed_files += 1
            print(f"Completed processing file: {filename}")
            
            # Save intermediate progress to a temporary file every 5 files
            if processed_files % 5 == 0:
                temp_output_path = f'../data/output/processed_index_temp_{timestamp}.csv'
                df_processed.to_csv(temp_output_path, index=False)
                ...
                print(f"Estimated time remaining: {est_time/60:.2f} minutes")
