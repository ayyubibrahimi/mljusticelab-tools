...
- Main topic 1
  • Sub-topic 1.1
  • Sub-topic 1.2
  • [ADDED] New sub-topic 1.3 from new summary

- Main topic 2
  • Sub-topic 2.1
  • Sub-topic 2.2
  • [ADDED] New sub-topic 2.3 from new summary

- Main topic 3
  • Sub-topic 3.1
  • Sub-topic 3.2
  • [ADDED] New sub-topic 3.3 from new summary
...
summary_template = """
<task_description>
Your task is to generate a comprehensive, bulletpoint summary of all the important information contained in the provided legal document excerpt. Extract all the key details presented in the current page.
</task_description>
...
<thinking_process>
Before summarizing, consider:
1. What essential information is contained in this document?
2. How can I organize the essential information contained in this document to maximize comprehensiveness and coherence? 
</thinking_process>
...
<output_instruction>
Generate the current page summary following the guidelines for legal documents.
</output_instruction>
"""
...
1. What essential information is contained in the summaries? 
2. Are there any contradictions or inconsistencies between summaries that need to be highlighted?
3. What information provides crucial context, and how can I ensure it's not lost in the combination?
4. Have I double-checked that no important information from either summary has been omitted?
</thinking_process>
...
Present the summary using the following structure:
- Legal Issues, Claims, and Charges
  • Bulletpoint 1.1 [summary 1]
  • Bulletpoint 1.2 [summary 2]
  • Bulletpoint 1.3 [summary 1]
...
- Main Findings, Decisions and Actions
  • Bulletpoint 1.1 [summary 2]
  • Bulletpoint 1.2 [summary 1]
  • Bulletpoint 1.3 [summary 1]

Note: Be highly selective. Include only the most important and impactful information under each header. 
Aim for no more than 10 bullet points per section, and use fewer if the additional points do not add significant value. 
Prioritize information that is crucial for understanding the core aspects of the case.
</output_format>
...
1. Have I identified all of the essential information from the summaries?
2. Have I added all of the missing essential information to the verified summary?
</thinking_process>
...
<output_format>
Present the verified summary using the existing structure of the current summary. Add new information from the memory log where appropriate, clearly marking additions with [ADDED] tags. For example:
- Legal Issues, Claims, and Charges
  • Bulletpoint 1.1 
  • Bulletpoint 1.2 
  • Bulletpoint 1.3 
...
<thinking_process>
Before and during the combination of summaries, consider:
1. What essential information is contined in the summaries? 
2. Are there any contradictions or inconsistencies between summaries that need to be highlighted?
3. What information provides crucial context, and how can I ensure it's not lost in the combination?
4. Have I double-checked that no important information from either summary has been omitted?
</thinking_process>
...
2. Information Preservation:
   • Ensure that ALL importafnt details from both summaries are accurately incorporated into the combined summary.
...
def output_files_exist(filename, output_directory, timestamp):
    base_name = os.path.splitext(filename)[0]
    output_files = [
        f"{timestamp}_{base_name}_memory_log.txt",
        f"{timestamp}_{base_name}_intermediate_summaries.txt",
        f"{timestamp}_{base_name}_final_summary.txt"
    ]
    return all(os.path.exists(os.path.join(output_directory, file)) for file in output_files)
...
if __name__ == "__main__":
    ...
    for filename in os.listdir(input_directory):
        if filename.endswith(".json"):
            ...
            if output_files_exist(filename, output_directory, timestamp):
