...
6. Limit the updated memory log to a maximum of 20 bullet points, focusing on the most vital information required to comprehend the content at any point in the police investigative files. If the memory log already has 20 bullet points, carefully consider whether any existing points should be removed or consolidated before adding new information.

7. Use clear, concise, and objective language to describe each point, ensuring that the information is easily understandable and serves as a quick reference for grasping the overall structure and content of the document. Maintain a neutral, unbiased perspective and avoid making assumptions or drawing conclusions beyond what is explicitly stated in the summaries.

8. If the new summary does not contain any information that significantly enhances the high-level understanding of the investigative files, directly addresses critical gaps in the current memory log, or contradicts existing information, it is crucial to return the current memory log without modifications.

9. When encountering inconsistencies or contradictions between the new summary and the existing memory log, carefully evaluate the reliability and credibility of the sources before making any changes. If the discrepancy cannot be resolved with confidence, prioritize maintaining the coherence and stability of the existing memory log.
...
Updated Memory Log (if necessary) or Current Memory Log:
"""
