...
def custom_sentence_split(text: str) -> List[str]:
    ...
    abbreviations = r"(?:[A-Za-z]\.){2,}|[A-Z][a-z]*\.|(?:Mr\.|Mrs\.|Ms\.|Dr\.|Prof\.|Inc\.|Ltd\.|Co\.|vs\.|et al\.|e\.g\.|i\.e\.|etc\.)"
    
    # Legal-specific abbreviations and terms
    legal_terms = r"(?:U\.S\.|U\.S\.C\.|U\.S\.C\.A\.|Fed\. Reg\.|F\.\d+d|S\.Ct\.|L\.Ed\.|F\.Supp\.|F\.R\.D\.|C\.F\.R\.)"
    
    # Define sentence ending punctuation
    sentence_end = r'[.!?]'
    
    # Combine all exceptions
    exceptions = f"(?:{abbreviations}|{legal_terms})"
    
    # Split the text, keeping separators
    segments = re.split(f'({sentence_end}|{exceptions})', text)
    ...
    for i, segment in enumerate(segments):
        ...
        if re.search(sentence_end + r'\s*$', segment) and not re.match(exceptions + r'\s*$', segment):
            # Look ahead to see if the next segment starts with a lowercase letter or a number
            if i + 1 < len(segments) and re.match(r'^\s*([a-z]|\d)', segments[i+1]):
                continue
    ...
    # Remove empty sentences and handle special cases
    sentences = [sent for sent in sentences if sent and len(sent.split()) > 3]
    
    # Merge sentences split by common legal citations and numbered lists
    merged_sentences = []
    for sent in sentences:
        if merged_sentences and (re.match(r'^\d+\s*[A-Za-z]', sent) or re.match(r'^[a-z]', sent)):
            merged_sentences[-1] += ' ' + sent
        else:
...
class AttentionBasedPrincipalComponents:
    ...
    def compute_attention_scores(self, sentence_embeddings: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            attention_output = self.attention(sentence_embeddings.unsqueeze(1))
            attention_weights = F.softmax(torch.matmul(attention_output, attention_output.transpose(1, 2)), dim=-1)
        return attention_weights.squeeze(1)

    def compute_contextual_importance(self, attention_scores: torch.Tensor, window_size: int = 3) -> torch.Tensor:
        n = attention_scores.shape[0]
        importance = torch.zeros(n)
        for i in range(n):
            start = max(0, i - window_size // 2)
            end = min(n, i + window_size // 2 + 1)
            local_attention = attention_scores[start:end, start:end]
            importance[i] = local_attention.sum() / local_attention.numel()
        return importance
    ...
    def compute_position_importance(self, num_sentences: int) -> torch.Tensor:
        return 1 - torch.abs(torch.linspace(-1, 1, num_sentences))

    def identify_principal_components(self, text: str, num_components: int = 3) -> str:
        sentences = custom_sentence_split(text)
        sentences = [s for s in sentences if len(s.split()) > 5 and not s.isdigit()]
        ...
        contextual_importance = self.compute_contextual_importance(attention_scores)
        bidirectional_importance = self.compute_bidirectional_importance(attention_scores)
        position_importance = self.compute_position_importance(len(sentences))
        
        # Combine different importance scores
        sentence_importance = contextual_importance + bidirectional_importance + 0.5 * position_importance
        ...
        for i, sentence in enumerate(sentences):
            if i in pc_indices:
                tagged_sentences.append(f"<review_for_context>{sentence}</review_for_context>")
            else:
...
def process_batch(batch, num_pages_to_concat, memory_log):
    ...
    # principal_component_identifier = AttentionBaseidPrincipalComponents()
    ...
    output_dir = "../data/output/test/augmented_texts"
    ...
    while i < len(batch):
        ...
        if concat_pages:
            ...
            try:
                ...
                augmented_text = original_document
