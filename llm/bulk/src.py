...
def write_json_output(filename, combined_summary, sentence_to_page):
    # Preparing data for output
    output_data = {
        "filename": filename,  # Ensure the filename is passed and used correctly
        "summary": combined_summary.get("page_content", ""),
        "total_pages": len(sentence_to_page)
    }
...
if __name__ == "__main__":
    ...
    for filename in os.listdir(input_directory):
        if filename.endswith(".json"):
            ...
            combined_summary = process_summaries(page_summaries, selected_model)
            # Pass the basename of the file to the output function
            basename = os.path.basename(json_path)
            write_json_output(basename, combined_summary, combined_summary['page_numbers'])
